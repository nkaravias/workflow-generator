...
def create_workflow_parser(subparsers):
    ...
    create_parser = workflow_subparsers.add_parser('create', help="Creates a workflow based on a workflow template and a list of changed files")
    ...
    create_parser = workflow_subparsers.add_parser('mock', help="Creates a mocked workflow based on a workflow template, where all deployments are marked as inactive")
    create_parser.add_argument('-o', '--output-file', type=str, help='Path to the output file', required=True)
    create_parser.add_argument('-l', '--log-level', choices=['info', 'debug', 'warning', 'error', 'critical'], default='info', help='Log level')
